"""Glue job: hourly SEQ aggregation (bronze/seq → silver/seq_agg_hour)."""
...
def main() -> None:
    ...
    config = PipelineConfig(
        name="seq-agg-hour",
        version="1.0.0",
        spark=SparkConfig(app_name="seq-agg-hour"),
        components=[
            ComponentConfig(
                name="read_bronze",
                component_type=ComponentType.SOURCE,
                class_path="glue_jobs.components.ReadBronzeDelta",
                config={"s3_bucket": args["s3_bucket"], "stream": "seq", "par_day": args["par_day"], "par_hour": args["par_hour"]},
            ),
            ComponentConfig(
                name="aggregate",
