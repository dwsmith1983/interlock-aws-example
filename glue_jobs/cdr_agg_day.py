"""Glue job: daily CDR aggregation (bronze/cdr → silver/cdr_agg_day)."""
...
def main() -> None:
    ...
    config = PipelineConfig(
        name="cdr-agg-day",
        version="1.0.0",
        spark=SparkConfig(app_name="cdr-agg-day"),
        components=[
            ComponentConfig(
                name="read_bronze",
                component_type=ComponentType.SOURCE,
                class_path="glue_jobs.components.ReadBronzeDelta",
                config={"s3_bucket": args["s3_bucket"], "stream": "cdr", "par_day": args["par_day"]},
            ),
            ComponentConfig(
                name="aggregate",
