...
- eval-false-pass: Write CHAOS#FALSE_PASS record to force PASS
...
def eval_false_pass(ctx):
    """Write CHAOS#FALSE_PASS record to force evaluator to return PASS."""
    table = ctx["table_name"]
    pipeline_id = ctx["pipeline_id"]
    now = ctx["now"]
    ttl = int(now.timestamp()) + (15 * 60)
    ...
    ddb.put_item(
        TableName=table,
        Item={
            "PK": {"S": f"CHAOS#FALSE_PASS#{pipeline_id}"},
            "SK": {"S": "ACTIVE"},
            "injectedAt": {"S": now.isoformat()},
            "ttl": {"N": str(ttl)},
        },
    )
    logger.info("wrote eval-false-pass for %s (expires in 15m)", pipeline_id)
    return {"action": "eval_false_pass", "pipeline": pipeline_id, "ttlMinutes": 15}
